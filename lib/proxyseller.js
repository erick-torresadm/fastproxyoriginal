...
const PERIODS = {
  '1w': { name: '1 Semana', days: 7, discount: 0 },
  '2w': { name: '2 Semanas', days: 14, discount: 0 },
  '1m': { name: '1 Mês', days: 30, discount: 0 },
  '2m': { name: '2 Meses', days: 60, discount: 0.1 },
  '3m': { name: '3 Meses', days: 90, discount: 0.15 },
  '6m': { name: '6 Meses', days: 180, discount: 0.25 },
  '12m': { name: '12 Meses', days: 365, discount: 0.35 }
};
...
class ProxySeller {
  ...
  getPricing(type, periodId, quantity) {
    ...
    const costUSD = basePrice * actualQty;
    const discount = period.discount || 0;
    const pricePerUnit = sellPrice * (1 - discount);
    const totalBRL = pricePerUnit * actualQty;
