...
const PRICING = {
    ...
    ipv6_fb: { sellPrice: 79.90, minQty: 1, swapsIncluded: 10 }
};
...
module.exports = {
    ...
    async createCheckoutSession(params) {
        ...
        console.log('Creating Stripe checkout:', {
            ...
            vipSupport,
            bonusSwaps,
            total: (calculation.total + (vipSupport ? 990 : 0)) / 100
        });
        ...
        const line_items = [
            {
                price_data: {
                    ...
                    product_data: {
                        ...
                        description: `${type === 'ipv6_fb' ? 'Kit Anunciante (10 rotações incluídas)' : `${quantity} proxies ${typeNames[type] || type}`}${calculation.periodDiscount > 0 ? ` - ${Math.round(calculation.periodDiscount * 100)}% desconto período` : ''}${calculation.couponDiscount > 0 ? ` - ${Math.round(calculation.couponDiscount * 100)}% cupom` : ''}${IS_TEST_MODE ? ' [TESTE]' : ''}`
                    },
                    unit_amount: calculation.unitAmount,
                    recurring: {
                        interval: period.includes('w') ? 'week' : 'month',
                        interval_count: period === '1w' ? 1 : (period === '2w' ? 2 : (calculation.periodMonths || 1))
                    }
                },
                quantity: quantity
            }
        ...
        if (vipSupport) {
            line_items.push({
                price_data: {
                    currency: 'brl',
                    product_data: {
                        name: '🚀 Suporte VIP 24h Prioritário',
                        description: 'Atendimento prioritário em até 15 minutos.'
                    },
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            mode: 'subscription',
            success_url: successUrl || `${appUrl}/success.html?session_id={CHECKOUT_SESSION_ID}`,
            cancel_url: cancelUrl || `${appUrl}/cancel.html`,
            metadata: {
                type: type,
                period: period,
                ...
                bonus_swaps: bonusSwaps.toString(),
                vip_support: vipSupport ? 'true' : 'false',
                test_mode: IS_TEST_MODE ? 'true' : 'false',
                plan_type: type === 'ipv6_fb' ? 'facebook_ads' : 'standard',
                swaps_included: (priceData.swapsIncluded || 0).toString()
            }
        });
